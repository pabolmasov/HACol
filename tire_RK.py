...
def sources(rho, v, u, g, ltot=0., dt=None):
    ...
    force = (-(g.sina*g.cth+g.cosa*g.sth)/g.r**2*(1.-gamedd)+omega**2*g.r*g.sth*g.cosa)*rho*g.across #*taufac
    beta = betafun(Fbeta(rho, u))
    ...
    irradheating = heatingeff * gamedd * (g.sina*g.cth+g.cosa*g.sth)/g.r**2*8.*pi*g.r*g.sth*afac*taufac # photons absorbed by the matter also heat it
    if(dt is not None):            
        trat = qloss * dt / u
        qloss *= tratfac(trat)
    else:
...
def derivo(m, s, e, l_half, s_half, p_half, fe_half, dm, ds, de):
    '''
...
def leftBC(m, s, e, dlleft, mdotleft = 0.):
    ...
    e0 = e[1]
    return m0, s0, e0
def rightBC(m, s, e, dlleft, edotright = 0.):
    s0 = -mdot
    m0 = m[-2]
    e0 = e[-2]
    return m0, s0, e0
...
def alltire():
    ...
    u = 3.*umagout+(rho/rho[-1])*0.01/g.r
    ...
    while(t<tmax):
        ...
        if(t>=tstore):
            ...
            if ifplot & (nout%plotalias == 0):
                print("plotting")
                plots.uplot(g.r, u, rho, g.sth, v, name=outdir+'/utie{:05d}'.format(nout))
